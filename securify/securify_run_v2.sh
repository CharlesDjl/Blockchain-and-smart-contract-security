...
output_directory="/home/logan/comp5566/Blockchain-and-smart-contract-security/securify/results_v2"

# Create the output directory if it doesn't exist
mkdir -p "$output_directory"

# Iterate over the .sol files and run securify in a Docker container
for input_file_path in "$input_directory"/*.sol; do
  ...
  start_time=$(date +%s.%N)
  output=$(sudo docker run -i -v "$input_directory:/share" securify "/share/$sol_file" 2>&1)
  exit_status=$?
  end_time=$(date +%s.%N)
